...
if [ -n "${HAVE_CAPABILITY_HDF5}" ]; then
    ADIOS2_HDF5_OPTS="-DHDF5_ROOT=${HDF5_DIR} -DADIOS22_USE_HDF5=ON"
else
    ADIOS2_HDF5_OPTS="-DADIOS22_USE_HDF5=OFF"
fi

# workaround for https://github.com/ornladios/ADIOS22/issues/3148
...
mkdir build
cd build
${CMAKE_DIR:+${CMAKE_DIR}/bin/}cmake -DADIOS22_USE_MPI=${ADIOS22_USE_MPI} ${ADIOS2_HDF5_OPTS} -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DCMAKE_INSTALL_LIBDIR=lib  -DBUILD_TESTING=OFF -DADIOS22_BUILD_EXAMPLES=OFF -DADIOS22_USE_Fortran=${ADIOS2_ENABLE_FORTRAN} -DADIOS22_USE_Python=OFF -DADIOS22_USE_ZeroMQ=OFF -DADIOS22_USE_PNG=OFF -DADIOS22_USE_BZip2=OFF -DADIOS22_USE_SST=${ADIOS2_ENABLE_SST} -DADIOS22_USE_BP5=${ADIOS2_USE_BP5} -DADIOS22_USE_CUDA=${ADIOS2_ENABLE_CUDA} -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=${ADIOS2_BUILD_TYPE} ..
